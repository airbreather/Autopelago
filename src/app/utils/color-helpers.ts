...
export function toRatDark(lightColor: TinyColor) {
...
export interface PixelColors {
  readonly color: TinyColor;
  readonly pixels: readonly Vec2[];
}
export function applyPixelColors(ctx: CanvasRenderingContext2D, ...pixelColors: PixelColors[]) {
  const dat = ctx.getImageData(0, 0, 64, 64);
  for (const { color, pixels } of pixelColors) {
